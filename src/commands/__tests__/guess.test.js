...
const { ONGOING, LOSE, GREEN, YELLOW, WHITE, WIN } = require('../../utils/constants')

const guess = require('../guess')
const messages = require('../../utils/messages')


jest.mock('../../configs/mode.json', () => ([{
    mode: 'wordle',
    maxTries: 6,
    wordArr: ['listA'],
...
describe('Guess', () => {
    ...
    // Mongodb data
    let staticData = new Keyv()

    // Redis session
    let index
    let sessionID
    let guessesID
    let session = new Keyv()
    ...
    test('Return one-line block and word when guess is incorrect', async () => {
        await session.set(sessionID, { active: true, guessesID, mode, status: LOSE })
        await session.set(guessesID, [])
        
        const res = await guess.execute({interaction, session, staticData})

        expect(res).toEqual(expect.stringContaining(interaction.options.guess.toUpperCase()))
    ...
    test('Return entire guesses block when guess is correct and hide word', async () => {
        ...
        expect(res).toEqual(expect.not.stringContaining(interaction.options.guess.toUpperCase()))
        expect(tileBuilder).toHaveBeenCalled()
    ...
    test('Failed when game is not started', async () => {
        await session.set(sessionID, { active: false, guessesID, mode, status: LOSE })
        const res = await guess.execute({interaction, session, staticData})
    ...
    test('Failed when out of move', async () => {
        await session.set(sessionID, { active: true, guessesID, mode, status: LOSE })
        await session.set(guessesID, new Array(6).fill([YELLOW, YELLOW, YELLOW, YELLOW]))
    ...
    test('Failed word length != 5', async () => {
        ...
        expect(res).toEqual(expect.stringContaining(messages.WORD_LENGTH_INVALID))
    ...
    test('Set status to lose after maxTries', async () => {
        ...
        await session.set(sessionID, { active: true, guessesID, mode, status: LOSE })
        await session.set(guessesID, new Array(5).fill([YELLOW, YELLOW, YELLOW, YELLOW]))
